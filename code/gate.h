...
void gate4(int gat4) {
	...
	if (x > 0) {
		if (remove("save.txt") != 0)
			std::cout << endl << endl << endl << "ОШИБКА УДАЛЕНИЯ ФАЙЛА СОХРАНЕНИЯ" << endl << endl << endl << endl;
		else
			std::cout << endl << endl << endl << "ФАЙЛЫ СОХРАНЕНИЯ УДАЛЕНЫ УСПЕШНО" << endl << endl << endl << endl;
		int main();
	}
}